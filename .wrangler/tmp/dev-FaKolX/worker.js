...
// .wrangler/tmp/bundle-DA81Xz/checked-fetch.js
  ...
  async search(query, options, env) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        lastError = error;
        const isNetworkError = error.name === "TypeError" || error.message.includes("fetch") || error.message.includes("network") || error.message.includes("ECONNRESET") || error.message.includes("ENOTFOUND");
...
var AdultMediaProvider = class {
  ...
  normalizeResults(results, options) {
    return results.map((item) => ({
      ...
      snippet: item.source || "",
      published_at: null,
      // This data is not available from this API
...
// src/handlers/provider-selftest.js
var ERROR_CODES = {
...
async function handleProviderSelfTest(request, env) {
  const url = new URL(request.url);
...
__name(handleProviderSelfTest, "handleProviderSelfTest");
...
__name(createProviderInstances, "createProviderInstances");
...
__name(runProviderHealthCheck, "runProviderHealthCheck");
async function testSingleProvider(providerName, provider, env, testQuery, debug) {
  ...
  try {
    const testOptions = {
      ...
      safeMode: true,
      ledger: null,
      // Bypass ledger for self-test
      cache_written: false
      // Ensure no cache writes
    };
    const results = await provider.search(testQuery, testOptions, env);
    const samplePayload = results.length > 0 ? redactSecrets(JSON.stringify(results[0]).substring(0, 800)) : null;
    return {
      provider: providerName,
      status: "ok",
...
__name(testSingleProvider, "testSingleProvider");
...
__name(mapProviderError, "mapProviderError");
function redactSecrets(payload) {
  if (!payload) return null;
  return payload.replace(/("key"\s*:\s*)"[^"]*"/gi, '$1"[REDACTED]"').replace(/("token"\s*:\s*)"[^"]*"/gi, '$1"[REDACTED]"').replace(/("api_key"\s*:\s*)"[^"]*"/gi, '$1"[REDACTED]"').replace(/("Authorization"\s*:\s*)"[^"]*"/gi, '$1"[REDACTED]"').replace(/("X-API-KEY"\s*:\s*)"[^"]*"/gi, '$1"[REDACTED]"').replace(/("x-rapidapi-key"\s*:\s*)"[^"]*"/gi, '$1"[REDACTED]"');
}
...
function generateConsolidatedReport(results) {
  const summary = {
    ...
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    test_duration_ms: Date.now() - (/* @__PURE__ */ new Date()).getTime()
...
__name(generateConsolidatedReport, "generateConsolidatedReport");
function generateHumanReadableReport(results, summary) {
  let report = `\u{1F680} JACK PORTAL PROVIDER HEALTH CHECK REPORT
`;
  report += `\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
...
`;
  report += `\u{1F4CB} PROVIDER STATUS
`;
  report += `\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
`;
...
`;
  ...
  report += `\u{1F527} TROUBLESHOOTING TIPS
`;
  report += `\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
`;
...
__name(generateHumanReadableReport, "generateHumanReadableReport");
...
// .wrangler/tmp/bundle-DA81Xz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DA81Xz/middleware-loader.entry.ts
